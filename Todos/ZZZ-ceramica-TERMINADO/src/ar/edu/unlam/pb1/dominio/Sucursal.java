...
if (this.pedidos[i].getCeramicas()[j] == null && cantidadDeCajasdeCeramicasAgregadas <= cantidadCajas) {
						this.pedidos[i].getCeramicas()[j] = cajaDeCeramica; // De Esta forma no se si tengo que utilizar
																			// el metodo de la clase Pedido												// agregarCajaDeCeramica o
	...
	public double obtenerPromedioDePrecioDeCajasDeCeramicaDeUnTipoDeCeramicaEnUnPedido(int numeroPedido,
		...
		for (int i = 0; i < this.pedidos.length; i++) {
			...
			for (int j = 0; j < this.pedidos[i].getCeramicas().length; j++) {
if (this.pedidos[i].getCeramicas()[j] != null && this.pedidos[i].getCeramicas()[j].getTipoDeSeramica().equals(tipoDeCeramica)) {
...
if (this.ceramicasDisponibles[j].getTipoDeSeramica().compareTo(this.ceramicasDisponibles[j+1].getTipoDeSeramica()) > 0) {
