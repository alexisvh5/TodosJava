...
public class GestionDeTienda {
	...
	private static void crearVentaDeProductosOServicios(Tienda tienda) {
		...
		String nombre = ingresarString("ingrese el nombre del cliente: ");
		String vendedor = ingresarString("ingrese el nombre del vendedor: ");
		Vendible[] vendiblesParaLaVenta = new Vendible[1000];
		Vendible vendibleParaLaVenta = null;
		String codigo = " ";
		char caracter = 'a';
	
		do {
			
		codigo = ingresarString("ingersa el/los codigo del vendible, una vez terminado");
		mostrarVendibles(tienda.getVendibles()); 
		
		vendibleParaLaVenta = tienda.obtenerVendiblePorCodigo(codigo);
		
		if (tienda.obtenerVendiblePorCodigo(codigo) !=null) {
			for (int i = 0; i < vendiblesParaLaVenta.length; i++) {
				if (vendiblesParaLaVenta[i] == null) {
					vendiblesParaLaVenta[i] = vendibleParaLaVenta;
		  }
		...
		} while (caracter != 's' && vendiblesParaLaVenta.length<1000 );
	...
	private static TipoDeVendible obtenerOpcionDeEnumParaTipoDeVendible() {
		...
		do {
			...
			if (numeroIngresadoTipoVendible < 1 || numeroIngresadoTipoVendible > TipoDeVendible.values().length) {
