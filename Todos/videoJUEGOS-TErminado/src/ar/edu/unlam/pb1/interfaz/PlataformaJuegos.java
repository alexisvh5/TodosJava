...
public class PlataformaJuegos {
	...
	public static void main(String[] args) {
		...
		do {
			mostrarPorPantalla("Bienvenido, que deseas hacer en la plataforma de juegos?");
...
(numeroIngresado < (MenuPrincipal.INICIAR_SESION.ordinal() + 1)	|| numeroIngresado > (MenuPrincipal.SALIR.ordinal() + 1));
			...
			case REGISTRARME:
				...
				break;
			case SALIR:
				mostrarPorPantalla("Esperamos que le haya servido la informacion, esperamos su llamado, gracias!. ");
				break;
			}
	...
	private static void registrarme(Plataforma plataforma) {
		...
		do {
			...
			correo = ingresarString("ingrese el Correo electrónico: ");
	...
	private static void iniciarSesionEn(Plataforma plataforma) {
		...
		if (nuevoUsuario != null) { /// por lo menos uno esta registrado
			mostrarPorPantalla("\n\nIniciemos sesion!");
...
(numeroIngresado < (MenuUsuario.MIS_JUEGOS.ordinal() + 1)|| numeroIngresado > (MenuUsuario.SALIR.ordinal() + 1));
	...
	opcion = ingresarNumeroEntero("\nDesea jugar? \n1) Si \n2) No");
			...
			case TIENDA:
				...
				do {
					...
					} while (opcion < 1 || opcion > 10 || cierto == false || opcion != 99);
