...
///1 a>z creciente  -1 z>a decreciente -- dos obj -- interfaz compare able
	}
	...
	private static void vender(Sucursal sucursal) {
		...
		do {
			...
			if (prodBuscado != null && cantidadAVender != 0
					&& sucursal.buscarStockProductoPorCodigoDeBarras(codigo).getCantidad() >= cantidadAVender) {

				if (sucursal.quitarProductoAlStock(sucursal.buscarProductoPorCodigoDeBarras(codigo), cantidadAVender)) {

					Venta nuevaVenta = new Venta(); ////

					nuevaVenta.agregarProducto(prodBuscado, cantidadAVender); ////

					sucursal.agregarVenta(nuevaVenta); //////
